...
{
    public class TR_FORM1_GDR_BUDGET_DA
    {
        ...
        public void Insert(TR_FORM1_GDR_BUDGET trform1gdrbudget)
        {
        ...
        public void Update(TR_FORM1_GDR_BUDGET trform1gdrbudget)
        {
            string newId = "Berhasil";
            try
            {
        ...
        public void DeleteByKey(String ID_BUDGET)
        {
            ...
            try
            {
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataSet dataSet = new DataSet();
                using (SqlCommand command = new SqlCommand(string.Format("DELETE FROM TR_FORM1_GDR_BUDGET  WHERE ID_BUDGET = @ID_BUDGET"), CnString))
                {
                    command.CommandType = CommandType.Text;
                    command.Parameters.Add("@ID_BUDGET", ID_BUDGET);
                    CnString.Open();
        ...
        public void DeleteFilter(String Where)
        {
            ...
            try
            {
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataSet dataSet = new DataSet();
                using (SqlCommand command = new SqlCommand(string.Format("DELETE  FROM TR_FORM1_GDR_BUDGET WHERE {0} ", Where), CnString))
                {
                    CnString.Open();
