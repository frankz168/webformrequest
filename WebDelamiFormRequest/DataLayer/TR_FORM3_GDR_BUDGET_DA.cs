...
{
    public class TR_FORM3_GDR_BUDGET_DA
    {
        ...
        public virtual DataSet GetDataAll()
        {
            ...
            using (SqlCommand command = new SqlCommand(string.Format("SELECT * FROM TR_FORM3_GDR_BUDGET"), CnString))
            {
        ...
        public virtual DataSet GetDataByKey(String ID_BUDGET)
        {
            SqlDataAdapter adapter = new SqlDataAdapter();
            DataSet dataSet = new DataSet();
            using (SqlCommand command = new SqlCommand(string.Format("SELECT * FROM TR_FORM3_GDR_BUDGET WHERE ID_BUDGET = @ID_BUDGET"), CnString))
            {
                command.CommandType = CommandType.Text;
        ...
        public virtual DataSet GetDataFilter(String Where)
        {
            SqlDataAdapter adapter = new SqlDataAdapter();
            DataSet dataSet = new DataSet();
            using (SqlCommand command = new SqlCommand(string.Format("SELECT * FROM TR_FORM3_GDR_BUDGET WHERE {0} ", Where), CnString))
            {
                CnString.Open();
        ...
        public void Insert(TR_FORM3_GDR_BUDGET trform1gdrbudget)
        {
            string newId = "Berhasil";
            try
            {

                string query = String.Format("INSERT INTO TR_FORM3_GDR_BUDGET(NO_FORM, KODE_BUDGET, NAMA_BUDGET, DETAIL, KET, ISPILIH, PIC) VALUES (@NO_FORM, @KODE_BUDGET, @NAMA_BUDGET, @DETAIL, @KET, @ISPILIH, @PIC)");
                CnString.Open();
        ...
        public void Update(TR_FORM3_GDR_BUDGET trform1gdrbudget)
        {
            string newId = "Berhasil";
            try
            {

                string query = String.Format("UPDATE TR_FORM3_GDR_BUDGET SET NO_FORM = @NO_FORM, KODE_BUDGET = @KODE_BUDGET, NAMA_BUDGET = @NAMA_BUDGET, DETAIL = @DETAIL,  KET = @KET, ISPILIH = @ISPILIH, PIC = @PIC WHERE ID_BUDGET = @ID_BUDGET ");
                CnString.Open();
        ...
        public void DeleteByKey(String ID_BUDGET)
        {
            ...
            try
            {
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataSet dataSet = new DataSet();
                using (SqlCommand command = new SqlCommand(string.Format("DELETE FROM TR_FORM3_GDR_BUDGET  WHERE ID_BUDGET = @ID_BUDGET"), CnString))
                {
                    command.CommandType = CommandType.Text;
                    command.Parameters.Add("@ID_BUDGET", ID_BUDGET);
                    CnString.Open();
        ...
        public void DeleteFilter(String Where)
        {
            ...
            try
            {
                SqlDataAdapter adapter = new SqlDataAdapter();
                DataSet dataSet = new DataSet();
                using (SqlCommand command = new SqlCommand(string.Format("DELETE  FROM TR_FORM3_GDR_BUDGET WHERE {0} ", Where), CnString))
                {
                    CnString.Open();
